...
namespace Interpreter {
	class parser {
		...
		public List<stmt> parse(List<token> _tokens) {
			tokens = _tokens;
			current = 0;
		...
		private expr assignment() {
			...
			//we have to check for multiple different assigment operators
			bool matchToken = match(TokenType.EQUAL) ||
				match(TokenType.P_INCREMENT) ||
				match(TokenType.N_INCREMENT) ||
			...
			if(matchToken) {
				...
				switch(equals.type) {
					...
					case TokenType.PLUS_EQUALS:
						value = new binaryExpr(Expr, new token(TokenType.PLUS, "+", null, equals.line), expression());
						break;
					case TokenType.MINUS_EQUALS:
						value = new binaryExpr(Expr, new token(TokenType.MINUS, "-", null, equals.line), expression());
						break;
					case TokenType.STAR_EQUALS:
						value = new binaryExpr(Expr, new token(TokenType.STAR, "*", null, equals.line), expression());
						break;
					case TokenType.SLASH_EQUALS:
						value = new binaryExpr(Expr, new token(TokenType.SLASH, "/", null, equals.line), expression());
						break;
					default:
		...
		private expr and() {
			expr Expr = equality();
			//this allows stuff like condition1 and condition2 and condition3
